...
def calc_nu(N, T, M, M_r, M_resc, M_doub_resc, M_sm, X, X_r, X_resc, X_doub_resc, X_sm,
    ...
    plt.plot(T, M_curve(T), color='IndianRed', label=r'$M(N = %s)$' %N)
    plt.errorbar(T, M, yerr = M_err, color='IndianRed', linestyle = 'None', capsize = 5, markeredgewidth=2)

    plt.scatter(T, M_r, s=50, marker='o', color='IndianRed')
    plt.plot(T, M_curve_r(T), '--', color='IndianRed', label=r'$M(N = %s)$' %(2*N))
    plt.errorbar(T, M_r, yerr = M_r_err, color='IndianRed', linestyle = 'None', capsize = 5, markeredgewidth=2)

    plt.scatter(T, M_resc, s=50, marker='o', color='RoyalBlue')
    plt.plot(T, M_curve_resc(T), color='RoyalBlue', label=r'$M_{resc}(N = %s)$' %N)
    plt.errorbar(T, M_resc, yerr = M_resc_err, color='RoyalBlue', linestyle = 'None', capsize = 5, markeredgewidth=2)

    plt.xlabel("Температура", fontsize=24)
    plt.ylabel("Намагниченность", fontsize=24)
    ...
    plt.plot(T, X_curve(T), color='IndianRed', label=r'$\chi(N = %s)$' %N)
    plt.errorbar(T, X, yerr = X_err, color='IndianRed', linestyle = 'None', capsize = 5, markeredgewidth=2)

    plt.scatter(T, X_r, s=50, marker='o', color='IndianRed')
    plt.plot(T, X_curve_r(T), linestyle='--', color='IndianRed', label=r'$\chi(N = %s)$' %(2*N))
    plt.errorbar(T, X_r, yerr = X_r_err, color='IndianRed', linestyle = 'None', capsize = 5, markeredgewidth=2)

    plt.scatter(T, X_resc, s=50, marker='o', color='RoyalBlue')
    plt.plot(T, X_curve_resc(T), color='RoyalBlue', label=r'$\chi_{resc}(N = %s)$' %N)
    plt.errorbar(T, X_resc, yerr = X_resc_err, color='RoyalBlue', linestyle = 'None', capsize = 5, markeredgewidth=2)

    plt.xlabel("Температура", fontsize=20)
    plt.ylabel("Восприимчивость", fontsize=20)
    plt.grid(linestyle='--', linewidth=1)
    plt.axis('tight')

    plt.legend(fontsize=18)
    plt.savefig("plots\\gamma_exponent.png")
